...
.head-lockup{display:flex;flex-direction:column;align-items:flex-start;gap:var(--space-2);min-width:0;margin-bottom:var(--space-3)}
.head-mission{margin:0;font-size:var(--font-xs);font-weight:var(--font-weight-semibold);line-height:1.3;letter-spacing:.12em;
...
.brand-lockup{height:40px;color:var(--foreground)}
...
.section-icon{flex:none;width:1.1em;height:1.1em;color:var(--primary-accent);order:-1}
...
background:var(--neutral-900);border:1px solid rgb(255 255 255 / .08);border-radius:var(--radius-full);
...
.deck-foot .brand-lockup{height:24px;color:var(--foreground)}
...
.deck-epam{display:flex;align-items:center;padding-right:16px;border-right:1px solid var(--border)}
.deck-epam .epam-word{height:18px;color:var(--foreground)}
.deck-team-code{color:var(--primary-accent);font-weight:var(--font-weight-semibold)}
...
.hero-top .brand-lockup{height:44px;color:var(--hero-fg)}
.epam-word{display:block;width:auto;flex:none}
.hero-top .epam-word{height:30px;color:var(--hero-fg)}
.hero-top .epam-word+.brand-word{padding-left:20px;border-left:1px solid var(--hero-line);height:40px;box-sizing:content-box}
...
/* A logo that comes from the configuration as markup. */
function deckMark(svg){ var t=document.createElement("template"); t.innerHTML=svg; return t.content.firstElementChild||document.createTextNode(""); }
...
function heroBuild(o){
  ...
  if(SLIDES.epam) top.appendChild(deckMark(SLIDES.epam));
...
function slideSources(){
  ...
  var cover=SLIDES.cover&&document.querySelector(SLIDES.cover), name="", code="", teamPeople=[];
  if(cover){
  ...
  Array.prototype.forEach.call(document.querySelectorAll(SLIDES.parts||"none"),function(part){
    ...
    /* A stage is remembered by its kind, not its place, so deleting one above it does not hide another. */
    var kinds={};
    rows(list).forEach(function(row, i){
      var kind=SLIDES.splitKey&&get(SLIDES.split+"["+i+"]."+SLIDES.splitKey), key=SLIDES.split+":"+(kind||i);
      if(kind){ kinds[kind]=(kinds[kind]||0)+1; if(kinds[kind]>1) key+="-"+kinds[kind]; }
      ...
      add(slidePack(wrap, head), key, (st&&textOf(st).trim())||(label+" "+(i+1)), sec, sec);
    });
  });
  if(SLIDES.thanks){
    var byeBase={name:SLIDES.thanks.title||"Thank you", code:"", lead:SLIDES.thanks.text||"", facts:[[SLIDES.thanks.team||"Team", name+(code?" · "+code:"")]].filter(function(f){ return name; })};
    /* The closing slide names the people too: with roles while they fit, else by name. */
    var bye=null;
    if(SLIDES.thanks.people&&teamPeople.length) [true,false].some(function(roles){
      var h=heroBuild({name:byeBase.name, code:"", lead:byeBase.lead, facts:byeBase.facts, people:teamPeople, roles:roles});
      if(h.peopleH<=HERO_PEOPLE||!roles){ bye=h; return true; }
      return false;
    });
    if(!bye) bye=heroBuild(byeBase);
...
function deckOpen(){
  ...
  deck.innerHTML='<div class="deck-slide"><div class="deck-head"></div><div class="deck-main"><div class="deck-body"></div></div>'+
    '<div class="deck-foot"><span class="deck-brand"></span><span class="deck-team"></span><span class="deck-epam"></span><span class="deck-no"></span></div>'+
  ...
  var teamCode=slideValue(cov&&SLIDES.coverSub&&cov.querySelector(SLIDES.coverSub));
  var team=deck.querySelector(".deck-team");
  if(who||SLIDES.eyebrow){
    team.appendChild(heroEl("span","deck-team-name",who||SLIDES.eyebrow));
    if(who&&(SLIDES.eyebrow||teamCode)){
      var sub=heroEl("span","deck-team-sub");
      if(teamCode) sub.appendChild(heroEl("span","deck-team-code",teamCode));
      if(teamCode&&SLIDES.eyebrow) sub.appendChild(document.createTextNode(" · "));
      if(SLIDES.eyebrow) sub.appendChild(document.createTextNode(SLIDES.eyebrow));
      team.appendChild(sub);
    }
  } else team.textContent=document.title||"";
  if(SLIDES.epam) deck.querySelector(".deck-epam").appendChild(deckMark(SLIDES.epam));
  else deck.querySelector(".deck-epam").hidden=true;
